...
/**
 * Field handler for Visto DSGA status.
 *
 * @ingroup views_field_handlers
 *
 * @ViewsField("entypa_praxis_visto_dsga")
 */
class VistoDsgaField extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    ...
    // Get visto_dsga_stato - read from DB
    $value = $this->statoIstruttoria($values, 'visto_dsga_stato', 0);
    ...
    // This applies to both normal workflow and acquisition workflow
    if (\Drupal::currentUser()->hasPermission('visto dsga') &&
        $editable &&
        !$nodata &&
        $concedibile == 1 &&
        $usa_visto_dsga &&
      ...
      $url = Url::fromRoute('entypa_praxis.visto_dsga', [
        'node' => $nid,
        'submission' => $sid,
        'tid' => $tid,
      ], [
        ...
        'attributes' => [
          ...
          'data-drupal-link-system-path' => 'node/' . $nid . '/submission/' . $sid . '/istanza-visto-dsga/' . $tid,
        ],
      ]);
      ...
      return [
        '#type' => 'container',
        '#attributes' => ['id' => 'dsga' . $sid],
        'link' => $link,
      ];
    }
    elseif ($nodata || $value == 0 || !$usa_visto_dsga) {
      return ['#markup' => '<span id="dsga' . $sid . '">---</span>'];
    }
    else {
      ...
      if (file_exists($icon_path)) {
        ...
        return [
          '#markup' => '<span id="dsga' . $sid . '"><img src="' . $icon_url . '" alt="' . $alt_text . '" title="' . $alt_text . '" style="width: 20px; height: 20px;" /></span>',
        ];
      }
      else {
        // Fallback to emoji
        ...
        return [
          '#markup' => sprintf(
            '<span id="dsga%d" title="%s" style="font-size: 20px;">%s</span>',
            $sid,
            $alt_text,
            $icon
          ),
